...
<?php 
echo '<h3>Oferta Efectiva de Doctores Conicet, seg&uacute;n Subdisciplina y Provincia del Lugar de Trabajo:</h3></br>';
	...
	echo '</br>';
		...
		echo '<select id="states_comb" name="states_comb">';
			...
			foreach($V->all_states_comb as $state)
				echo '<option value="'.$state->state.'">'.$state->state.'</option>';
		echo '</select>&nbsp;';
		echo 'Disponibilidad de cambio de provincia&nbsp;';
		echo '<input type="checkbox" name="moving" id="moving"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
