<?php
require_once ('models/cases_model.php');
class cases_page extends tina_mvc_base_page_class {
    ...
    function __construct( $request=array() ) {
        ...
        if ($task == 'add'){
			...
			wp_redirect(home_url().'?page_id='.$pageid.'/case/');
			exit;
		}
		if ($task == 'delete'){
		...
		if ($task == 'edit'){
			$tina_mvcv_pages = get_option("tina_mvc_pages");
			$pageid = $tina_mvcv_pages['masvalor']['page_id'];
			wp_redirect(home_url().'?page_id='.$pageid.'/case/&cid='.$cid);
			exit;
		}
		
        $this->cases( $request,$msg);
    }
	 	 
    function cases( $request,$msg) {
        global $current_user;
		get_currentuserinfo();
		$model = new casesModel();
		...
		if (!checkUserType($current_user->user_login,'masvalor-admin'))
			...
			$this->set_post_title(__('Casos'));
			$this->set_post_content( $this->load_view('cases', $tpl_vars ) );
		}	
    }
