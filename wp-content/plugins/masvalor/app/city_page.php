...
class city_page extends tina_mvc_base_page_class {
    ...
    function __construct( $request=array() ) {
		...
		if ($task == 'save'){
			...
			$state = $_POST['state'];
			$msg = $model->save($cid,$name,$country,$state);			
        ...
        $this->addcity( $request,$msg);
    }
	 	 
    function addcity( $request,$msg) {
        global $current_user;
		get_currentuserinfo();
		$model = new cityModel();
		...
		if (!checkUserType($current_user->user_login,'masvalor-admin'))
			...
			$this->set_post_content( $this->load_view('city', $tpl_vars ) );
		}
