 ...
 color:black;
			...
			<div id="table_noticia" style="font-size: 14px;margin-left: -35px;">
				<form action="" method="post" name="tinymce" id="adminForm">		
				 <h2 style="margin-left:13px;"><?php echo __('B&uacute;squedas') ?></h2>
				 <br/>
				<table> 
				 <tr>
					 <td>
						...
						<div>
							  <td style="width:100%;padding-left: 11px;"><?php echo __('Filtro') ?>
								...
								<select name="filter_sel" id="filter_sel" style="height: 26px;padding-top: 1px;">
							   ...
							   <button onclick="this.form.submit();" style="padding-top: 1px;">Buscar</button> 
							   <button style="padding-top: 1px;" onclick="document.getElementById('search').value='';this.form.getElementById('filter_state').value='-1';this.form.submit();">Reset</button>
						       <a href="javascript:filterAdvanced()" title="Filtro Fechas">
							    <img src ="wp-content/plugins/masvalor/app/includes/image/dates.png" style="position: absolute; margin-left: 6px;" alt="Filtro fechas"></a>
						...
						<div style="width:100%">
							<hr/>
							  ...
							  <?php if (checkUserType($current_user->user_login,'masvalor-admin')) { ?>
				...
				<div class="paginator" style="margin-left:16px">
					...
					if ($pages > 1)
						for ($i=1;$i<=$pages;$i++){
							$pageLink = masvalor_getUrl().'/searches/&limitstart='.(($i-1)*$V->itemsPerPage);
							if ($V->currPage != $i)
